...
interface ConnectionInterface {
    ...
    /**
     * 执行一条<b>SELECT</b>查询语句，并返回所有记录（如果有）
     * 
     * @param string $sql 带占位符的SQL语句
     * @param array $parameters 绑定参数。参数个数必须和SQL语句中的占位符数量一样多
     * @return array|null|false 成功则返回相应的数据，没有符合条件的记录则返回null，
     ...
     * 禁用“从服务器”
     * 
     * 执行此方法会导致该连接上的所有<b>读</b>操作都在master上进行，直到调用了enableSlaves()方法为止。
     * 该方法一般和enableSlaves()方法一起使用。
     * 
     ...
     * 启用“从服务器”
     * 
     * 执行此方法会恢复之前的读写分离模式，该方法一般和disableSlaves()方法一起使用。
     * 
     * 注意：这只对配置了主从模式的架构起作用，对于不支持主从模式的架构，则在此方法里什么都不做
     */
